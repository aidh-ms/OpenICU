...
class DeviceUsageExtractor(PandasDatabaseMixin, ConceptExtractor[FHIRObjectDeviceUsage]):
    ...
    def extract(self) -> DataFrame[FHIRObjectDeviceUsage] | None:
        ...
        device_usage_df[FHIRObjectDeviceUsage.patient] = df.apply(self._apply_subject, axis=1)
        device_usage_df[FHIRObjectDeviceUsage.timing_date_time] = df.apply(self._apply_timing_date_time, axis=1)
        device_usage_df[FHIRObjectDeviceUsage.device] = df.apply(self._apply_device, axis=1)
        device_usage_df[FHIRObjectDeviceUsage.status] = df.apply(self._apply_status, axis=1)

        return device_usage_df.pipe(DataFrame[FHIRObjectDeviceUsage])
