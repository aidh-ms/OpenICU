    ...
    def extract(self) -> None:
        ...
        for table in self._registry.values():
            ...
            try:
                ...
                persist_output_path = Path(self._workspace_dir.path).parent / "persist"
                persist_output_path.mkdir(parents=True, exist_ok=True)

                persist_output_file = persist_output_path / f"{table.name}.parquet"
                tmp_persist_output_file = persist_output_file.with_suffix(".tmp.parquet")

                logger.info(
                    "Persisting table %s to %s",
                    table.name,
                    persist_output_file,
                )

                if persist_output_file.exists():
                    logger.info(
                        "Existing persist output found for table %s, using it",
                        table.name,
                    )
                else:
                    if tmp_persist_output_file.exists():
                        tmp_persist_output_file.unlink()

                    lf.sink_parquet(
                        tmp_persist_output_file,
                        maintain_order=False,
                    )

                    tmp_persist_output_file.replace(persist_output_file)

                # Important: continue from persisted Parquet, not from CSV LazyFrame
                del lf
                gc.collect()

                lf = pl.scan_parquet(persist_output_file)
