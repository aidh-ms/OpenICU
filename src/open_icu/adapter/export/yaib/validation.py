...
These functions are intentionally small and notebook-friendly. They validate the
Python/Polars conversion output before the final comparison against YAIB/RICU.
"""
...
def scan_dynamic(path: str | Path) -> pl.LazyFrame:
    """Scan a wide dynamic table parquet."""
    return pl.scan_parquet(path)


def table_summary(df: pl.LazyFrame) -> pl.DataFrame:
    """Return basic row/stay/time summary for a dynamic table."""
    return df.select(
        [
...
def validate_dynamic_columns(
    ...
    """Check whether expected YAIB/RICU dynamic columns are present."""
...
def concept_coverage(
    ...
    """Count non-null values per dynamic concept."""
    vars_ = dynamic_vars or DYNAMIC_VARS
...
def ricu_range_report(
    ...
    """Check whether final aggregated values are inside RICU min/max ranges."""
    vars_ = dynamic_vars or DYNAMIC_VARS
    actual_cols = set(df.collect_schema().names())
...
def debug_concept_against_output(
    *,
    ...
    """Trace one dynamic concept from raw OpenICU parquet to final wide output.

    Returns compact summaries plus the largest absolute differences between a
    manually recomputed concept table and the final output table.
    """
    ...
    if concept_file is None:
        raise FileNotFoundError(
            f"Could not find parquet for RICU concept {concept!r} / "
            f"OpenICU concept {openicu_concept!r}."
        )
    ...
    mapped = map_events_to_stays(filtered, stays)
    ...
    return {
        ...
        "worst_examples": comparison.sort("abs_diff", descending=True).limit(20).collect(),
    }
