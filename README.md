...
The shared concept dictionary in [`configs/concepts/`](configs/concepts/) currently covers ~90 concepts across vital signs, blood gas, clinical chemistry, hematology, medications (incl. vasopressors and antibiotics), neurological scores, respiratory parameters, fluid output, demographics, and diagnoses — the `diagnosis` concept harmonises mixed ICD-9/ICD-10 coding into a single ICD-10-CM vocabulary using the bundled CMS GEM crosswalk (fully offline).
