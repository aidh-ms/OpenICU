...
- The sharding step, step `hooks()`, and broader test coverage are open work.
- The docs dependency group only installs `mkdocs`, but `mkdocs.yaml` references material/mkdocstrings/macros/gen-files/literate-nav/section-index plugins — building the docs site needs those added.
