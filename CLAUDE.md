...
- **`steps/extraction/`** — reads raw Parquet/CSV/CSV.GZ tables per `TableConfig` (format from `type`, else inferred from the path extension, default parquet): typed `scan_parquet`/`scan_csv`, datetime parsing (native temporal columns used as-is, string columns parsed via `params`), lookup joins, then per event: column mapping to MEDS, code assembly as `dataset//table//code_prefix//code parts//code_suffix`, casts (`subject_id` Int64, `time` Datetime[us], `code` Str, `numeric_value` Float32, `text_value` Str + extension columns), sink to `workspace/<step>/<dataset>/<version>/<table>/<EVENT>.parquet` (appends if file exists). Callback/filter hook order: table `pre_callbacks` → `pre_filters` → datetime → `callbacks` → `filters`; per join `post_join_*`; table `post_join_*` → `transformations`; per event `pre_callbacks` → mapping → `callbacks` → `filters` → `transformations` → select/cast → `output_filters`.
- **`steps/concept/`** — harmonises extraction output into dataset-agnostic concepts. `ConceptConfig` (name, version, unit → output code `name//unit`) + per-dataset configs discovered by matching filename in `dataset_configs` paths. Three types (pydantic discriminated union on `type`): `simple` (regex on code column per mapping pattern), `derived` (depends on other concepts; dependency graph resolved with `graphlib.TopologicalSorter`; joins concept parquets), `complex` (dotted-path Python `ConceptTransformer`, built via `build_transformer(concept)` and called with the running `ConceptStep`). Output: `workspace/<step>/<concept name>/<version>/<dataset>.parquet`. Built-in transformers live in `steps/concept/transformers/`: `ICD9ToICD10Transformer` harmonises diagnoses to ICD-10-CM via the bundled CMS GEM crosswalk (`transformers/data/`, offline; deterministic first-choice mapping, unmappable codes kept under an `ICD9CM` namespace); shipped `diagnosis` concept mappings exist for mimic-iv and nwicu. `WindowedConceptTransformer` (in `transformers/windowed.py`) is a reusable base for concepts that align several dependency concepts onto one continuous per-subject timeline and carry each onto it with a windowed `Aggregation` (`Locf`, `RollingSum`/`Max`/`Min`/`Mean`, `Exists`), evaluate a `compute` expression, and emit at *trigger* timestamps (default: any input) — the primitive `derived`'s exact-key joins can't express (worst/mean/last-over-window, lookaround). The six SOFA components are thin `SofaComponent` subclasses (each = `inputs` + a piecewise `score`): renal (`creatinine` LOCF, 24h `urine_output` `RollingSum` where recorded `0` = anuria but missing ≠ zero), coagulation, liver, CNS, cardiovascular (MAP + vasopressor rates, LOCF), respiration (PaO₂/FiO₂ gated on ventilation, degrading when vent unknown). `WindowedSumTransformer` (declarative, `kwargs.terms`) sums LOCF-aligned inputs — used both for the total `sofa` (sum of the six sub-scores; not a plain `derived` sum because components fire at different timestamps) and for a derived `GCS_total` on mimic (eye+motor+verbal). Shipped `sofa*` mappings: full set for mimic-iv and eicu-crd (eicu respiration lacks ventilation → degrades), coagulation+liver+renal for aumc. `SuspectedInfectionTransformer` (`transformers/sepsis.py`) is ricu's Sepsis-3 `susp_inf`: antibiotic-first→culture-within-24h / culture-first→antibiotic-within-72h. Detected at the *later* event via trailing windows, but each input is carried as `LastEventTime` (partner's most recent event time in the window) so the event is emitted once, valued 1, dated at the *onset* = `min(antibiotic, culture)` (the earlier event, per Seymour — the anchor for a SOFA window) via the base's `event_time()` override; shipped for mimic-iv only (the only dataset with both `antibiotics` and `body_fluid_sampling`). Presence in the base is event-existence, not `numeric_value` non-null, so marker/boolean concepts without a value work (`Exists`/`LastEventTime` use `closed="both"`). The base drops rows where `compute` returns null (no value → no event) and deduplicates on output timestamp, so `event_time()` overrides collapse cleanly. Note: dependency refs resolve case-insensitively (`BaseConfig.ensure_prefix` lowercases to match the all-lowercase identifiers), so mixed-case concepts like `GCS_total` work as dependencies.
